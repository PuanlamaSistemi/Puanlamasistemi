            ...
            <div class="glass-card rounded-3xl p-6 md:p-8 flex flex-col h-full">
                ...
                <div class="overflow-y-auto pr-2 space-y-4 flex-1 max-h-[600px]">
                    ...
                    <?php else: ?>
                        ...
                        ?>
                            <div class="bg-white/60 p-5 rounded-2xl border border-white shadow-sm hover:shadow-md transition">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <?php if(isset($_SESSION['role']) && $_SESSION['role'] === 'jury'): ?>
                                            <form method="POST" action="vote.php?id=<?php echo urlencode($project['id']); ?>" class="inline ml-2" onsubmit="return confirm('Bu değerlendirmeyi silmek istediğinize emin misiniz?');">
                                                ...
                                                <button type="submit" class="text-red-400 hover:text-red-600 text-xs transition px-2 py-1 bg-red-50 rounded-lg"><i class="fa-solid fa-trash"></i> Sil</button>
                                            </form>
                                        <?php endif; ?>
                                    </div>
                                    <div class="bg-amber-100 text-amber-700 px-2 py-1 rounded-lg text-xs font-black flex items-center gap-1 border border-amber-200">
                                        <?php echo number_format($ind_avg, 1); ?> <i class="fa-solid fa-star text-[10px]"></i>
                                    </div>
                                </div>
                                <?php if(!empty($v['comment'])): ?>
                                    <p class="text-gray-700 text-sm mt-3 bg-white/50 p-3 rounded-xl border border-indigo-50 leading-relaxed italic">
